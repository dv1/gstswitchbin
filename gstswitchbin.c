...
static void gst_switch_bin_child_proxy_iface_init(gpointer iface, gpointer iface_data);
static GObject* gst_switch_bin_child_proxy_get_child_by_index(GstChildProxy *child_proxy, guint index);
static guint gst_switch_bin_child_proxy_get_children_count(GstChildProxy *child_proxy);
...
static void gst_switch_bin_set_property(GObject *object, guint prop_id, GValue const *value, GParamSpec *pspec);
static void gst_switch_bin_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);

static gboolean gst_switch_bin_sink_event(GstPad *pad, GstObject *parent, GstEvent *event);
static gboolean gst_switch_bin_sink_query(GstPad *pad, GstObject *parent, GstQuery *query);
static gboolean gst_switch_bin_src_query(GstPad *pad, GstObject *parent, GstQuery *query);
...
static void gst_switch_bin_child_proxy_iface_init(gpointer iface, G_GNUC_UNUSED gpointer iface_data)
{
	GstChildProxyInterface *child_proxy_iface = iface;

	child_proxy_iface->get_child_by_index = GST_DEBUG_FUNCPTR(gst_switch_bin_child_proxy_get_child_by_index);
	child_proxy_iface->get_children_count = GST_DEBUG_FUNCPTR(gst_switch_bin_child_proxy_get_children_count);
}
...
static void gst_switch_bin_class_init(GstSwitchBinClass *klass)
{
...
static void gst_switch_bin_init(GstSwitchBin *switch_bin)
{
	GstPad *pad;
	...
	switch_bin->sinkpad = gst_ghost_pad_new_no_target_from_template(
		"sink",
		gst_element_class_get_pad_template(
			GST_ELEMENT_GET_CLASS(switch_bin),
			"sink"
		)
	);
	gst_element_add_pad(GST_ELEMENT(switch_bin), switch_bin->sinkpad);

	switch_bin->srcpad = gst_ghost_pad_new_no_target_from_template(
		"src",
		gst_element_class_get_pad_template(
			GST_ELEMENT_GET_CLASS(switch_bin),
			"src"
	...
	switch_bin->input_identity = gst_element_factory_make("identity", "input-identity");
...
static gboolean gst_switch_bin_sink_event(GstPad *pad, GstObject *parent, GstEvent *event)
{
	...
	switch (GST_EVENT_TYPE(event))
	{
		...
		case GST_EVENT_CAPS:
		{
			...
			GST_DEBUG_OBJECT(switch_bin, "sink pad got caps event with caps %" GST_PTR_FORMAT " ; looking for matching path", (gpointer)caps);
		...
		default:
			GST_DEBUG_OBJECT(switch_bin, "sink event: %s", gst_event_type_get_name(GST_EVENT_TYPE(event)));
			return gst_pad_event_default(pad, parent, event);
	}
}


static gboolean gst_switch_bin_handle_query(GstPad *pad, GstObject *parent, GstQuery *query, char const *pad_name)
{
	GstSwitchBin *switch_bin = GST_SWITCH_BIN(parent);
	...
	switch (GST_QUERY_TYPE(query))
	{
		case GST_QUERY_CAPS:
		{
			...
			else if ((switch_bin->current_path == NULL) || (switch_bin->current_path->element == NULL))
			{
			...
			else
			{
				...
				GstPad *element_pad = gst_element_get_static_pad(switch_bin->current_path->element, pad_name);
		...
		case GST_QUERY_ACCEPT_CAPS:
		{
			...
			PATH_UNLOCK(switch_bin);
			GST_DEBUG_OBJECT(switch_bin, "%s accept_caps query:  acceptable: %d  caps: %" GST_PTR_FORMAT, pad_name, (gint)acceptable, (gpointer)caps);
...
static gboolean gst_switch_bin_set_num_paths(GstSwitchBin *switch_bin, guint new_num_paths)
{
	...
	{
		GST_DEBUG_OBJECT(switch_bin, "no change in number of paths - ignoring call");
		return TRUE;
	}
	else if (switch_bin->num_paths < new_num_paths)
	{
		...
		GST_DEBUG_OBJECT(switch_bin, "adding %u new paths", new_num_paths - switch_bin->num_paths);

		switch_bin->paths = g_realloc(switch_bin->paths, sizeof(GstObject *) * new_num_paths);
		...
		for (i = switch_bin->num_paths; i < new_num_paths; ++i)
		{
			...
			snprintf(name, sizeof(name), "path%u", i);
			switch_bin->paths[i] = path = g_object_new(gst_switch_bin_path_get_type(), "name", name, NULL);
			path->bin = switch_bin;

			gst_object_set_parent(GST_OBJECT(path), GST_OBJECT(switch_bin));
			gst_child_proxy_child_added(GST_CHILD_PROXY(switch_bin), G_OBJECT(path), name);

			GST_DEBUG_OBJECT(switch_bin, "added path #%u \"%s\" (%p)", i, name, (gpointer)path);
		}
	}
	else
	{
		...
		GST_DEBUG_OBJECT(switch_bin, "removing the last %u paths", switch_bin->num_paths - new_num_paths);

		for (i = new_num_paths; i < switch_bin->num_paths; ++i)
		{
			GstSwitchBinPath *path = switch_bin->paths[i];
			strncpy(name, GST_OBJECT_NAME(path), sizeof(name));
			...
			if (path == switch_bin->current_path)
			{
				...
				GST_DEBUG_OBJECT(switch_bin, "path #%u \"%s\" (%p) is the current path - selecting a new current path will be necessary", i, name, (gpointer)(switch_bin->paths[i]));
			}

			gst_child_proxy_child_removed(GST_CHILD_PROXY(switch_bin), G_OBJECT(path), GST_OBJECT_NAME(path));
			gst_object_unparent(GST_OBJECT(switch_bin->paths[i]));

			GST_DEBUG_OBJECT(switch_bin, "removed path #%u \"%s\" (%p)", i, name, (gpointer)(switch_bin->paths[i]));
		}
	...
	if (new_num_paths > 0)
	{
		if (cur_path_removed)
		{
			...
			if (switch_bin->last_caps != NULL)
			{
				GST_DEBUG_OBJECT(switch_bin, "current path was removed earlier - need to select a new one based on the last caps %" GST_PTR_FORMAT, (gpointer)(switch_bin->last_caps));
				return gst_switch_bin_select_path_for_caps(switch_bin, switch_bin->last_caps);
			}
			else
			{
...
static gboolean gst_switch_bin_select_path_for_caps(GstSwitchBin *switch_bin, GstCaps *caps)
{
	...
	if (path == NULL)
	{
		...
		GST_ELEMENT_ERROR(switch_bin, STREAM, WRONG_TYPE, ("could not find compatible path"), ("sink caps: %" GST_PTR_FORMAT, (gpointer)caps));
		ret = FALSE;
	}
	else
	{
...
static gboolean gst_switch_bin_switch_to_path(GstSwitchBin *switch_bin, GstSwitchBinPath *switch_bin_path)
{
	...
	if (switch_bin_path != NULL)
		GST_DEBUG_OBJECT(switch_bin, "switching to path \"%s\" (%p)", GST_OBJECT_NAME(switch_bin_path), (gpointer)switch_bin_path);
	else
		GST_DEBUG_OBJECT(switch_bin, "switching to NULL path (= disabling current path)");
	...
	if (switch_bin_path != NULL)
	{
		if (switch_bin_path->element != NULL)
		{
			...
			GstPad *pad = gst_element_get_static_pad(switch_bin_path->element, "src");
			if (pad == NULL)
			{
				GST_ERROR_OBJECT(switch_bin, "path element has no static srcpad - cannot link");
				ret = FALSE;
				goto finish;
			}
			...
			{
				GST_ERROR_OBJECT(switch_bin, "could not set the path element's srcpad as the ghost srcpad's target");
				ret = FALSE;
			}
			...
			if (!gst_element_link(switch_bin->input_identity, switch_bin_path->element))
			{
				GST_ERROR_OBJECT(switch_bin, "linking the path element's sinkpad failed ; check if the path element's sink caps and the upstream elements connected to the switchbin's sinkpad match");
				ret = FALSE;
				goto finish;
			}
			...
			{
				GST_ERROR_OBJECT(switch_bin, "could not sync the path element's state with that of the switchbin");
				ret = FALSE;
				goto finish;
			}
		...
		else
		{
			...
			if (!gst_ghost_pad_set_target(GST_GHOST_PAD(switch_bin->srcpad), srcpad))
			{
				GST_ERROR_OBJECT(switch_bin, "could not set the path element's srcpad as the ghost srcpad's target");
				ret = FALSE;
			}

			GST_DEBUG_OBJECT(switch_bin, "pushing stream-start downstream prior to enabling drop probe");
			gst_element_send_event(switch_bin->input_identity, gst_event_ref(switch_bin->last_stream_start));

			/* There is no path element. This means that this path is supposed to not
			 * let data through. Activate the drop probe to drop buffers. */
			gst_switch_bin_set_sinkpad_drop(switch_bin, TRUE);
...
static GstCaps* gst_switch_bin_get_allowed_caps(GstSwitchBin *switch_bin, G_GNUC_UNUSED gchar const *pad_name, GstCaps *filter)
{
	/* must be called with path lock held */
...
static void gst_switch_bin_set_sinkpad_block(GstSwitchBin *switch_bin, gboolean do_block)
{
	...
	if ((do_block && (switch_bin->blocking_probe_id != 0)) || (!do_block && (switch_bin->blocking_probe_id == 0)))
		return;
	...
	if (do_block)
	{
		switch_bin->blocking_probe_id = gst_pad_add_probe(pad, GST_PAD_PROBE_TYPE_BLOCK_DOWNSTREAM, gst_switch_bin_blocking_pad_probe, NULL, NULL);
	}
	else
	{
...
static GstPadProbeReturn gst_switch_bin_blocking_pad_probe(G_GNUC_UNUSED GstPad *pad, GstPadProbeInfo *info, G_GNUC_UNUSED gpointer user_data)
{
...
static void gst_switch_bin_set_sinkpad_drop(GstSwitchBin *switch_bin, gboolean do_drop)
{
	...
	if ((do_drop && (switch_bin->drop_probe_id != 0)) || (!do_drop && (switch_bin->drop_probe_id == 0)))
		return;
	...
	if (do_drop)
	{
		switch_bin->drop_probe_id = gst_pad_add_probe(pad, GST_PAD_PROBE_TYPE_DATA_DOWNSTREAM, gst_switch_bin_dropping_pad_probe, switch_bin, NULL);
	}
	else
	{
		gst_pad_remove_probe(pad, switch_bin->drop_probe_id);
		switch_bin->drop_probe_id = 0;
	}

	GST_DEBUG_OBJECT(switch_bin, "sinkpad drop enabled: %d", do_drop);

	gst_object_unref(GST_OBJECT(pad));
}


static GstPadProbeReturn gst_switch_bin_dropping_pad_probe(G_GNUC_UNUSED GstPad *pad, GstPadProbeInfo *info, G_GNUC_UNUSED gpointer user_data)
{
	GstSwitchBin *switch_bin = (GstSwitchBin *)user_data;
	...
	if (info->type & GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM)
	{
		...
		switch (GST_EVENT_TYPE(event))
		{
			...
			default:
				GST_DEBUG_OBJECT(switch_bin, "letting through %s event", GST_EVENT_TYPE_NAME(event));
				return GST_PAD_PROBE_OK;
		}
	}
...
static void gst_switch_bin_path_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);

static gboolean gst_switch_bin_path_use_new_element(GstSwitchBinPath *switch_bin_path, GstElement *new_element);



static void gst_switch_bin_path_class_init(GstSwitchBinPathClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS(klass);

	object_class->dispose       = GST_DEBUG_FUNCPTR(gst_switch_bin_path_dispose);
	object_class->set_property  = GST_DEBUG_FUNCPTR(gst_switch_bin_path_set_property);
	object_class->get_property  = GST_DEBUG_FUNCPTR(gst_switch_bin_path_get_property);
...
static void gst_switch_bin_path_init(GstSwitchBinPath *switch_bin_path)
{
